...
```bash
[bat] (~/42/ft_transcendence)$ tree -L3
├── Makefile
├── setup.sh
└── srcs
    ├── backend
...
```bash
[bat] (~/42/ft_transcendence/srcs/backend/proxy)$ tree
.
├── data
│   ├── package.json
│   ├── package-lock.json
...
```
-> Le dossier **./data/** présent dans chaque service est monté à la racine du container. quand on lance le container il va dans data, install les dépendances spécifiées dans package.json et il lance le service avec npm run dev (ca permet de voir les modifs en live).

-> Le dossier shared est un cas spécial. Chaque service est isolé des autres du fait de l'architecture du  backend mai on a quand meme besoin d'avoir des fichiers communs aux différents services (comme les définitions des types/classes).
Ils sont placés dans le dossier **/backend/shared/** qui sera monté à l'éxecution dans le dossier **data/src/shared** de chaque service.
Attention ducoup à bien modifier ces fichiers dans le dossier source **backend/shared** quand les containers ne sont pas lancés. Parce que si on modifie dans un service alors que le service est stoppé, quand on le lancera a nouveau, la copy du dossier source /backend/shared dans /*service/data/src/shared va surremtn ecraser les modifs. (A Verifier en vrais jsp)
...
```bash
[bat] (~/42/ft_transcendence/srcs/frontend)$ tree
├── data
│   ├── package.json
│   ├── package-lock.json
│   ├── postcss.config.js
